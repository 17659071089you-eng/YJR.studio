...
const Masonry = ({
  ...
  scaleOnHover = true,
  hoverScale = 0.95,
  blurToFocus = false, // Set to false as requested
  ...
  useLayoutEffect(() => {
    ...
    if (!hasMounted.current) {
      ...
      // If it's already mounted and just resizing, update positions with animation
      grid.forEach((item) => {
        const selector = `[data-key="${item.id}"]`;
    ...
    return () => {
      // No ScrollTrigger to kill anymore
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      {grid.map(item => {
        return (
          <div
            key={item.id}
            data-key={item.id}
            className="absolute will-change-transform p-2 cursor-pointer top-0 left-0"
          ...
          >
            ...
            >
              ...
              {colorShiftOnHover && (
                <div
                  className="color-overlay absolute inset-0 pointer-events-none rounded-xl opacity-0"
                  style={{
